...
### C16. "How would you handle a solar conjunction with 0 contact windows — not even the Lagrange relays have line-of-sight?"

This is a worst-case scenario beyond the standard conjunction model. If L4/L5 relays also have degraded geometry (e.g., during a particularly unfavourable conjunction where Mars is near the solar limb from both L4 and L5 perspectives), the strategy is: (1) **Pre-position everything**: T-21 days (earlier than the standard T-14), upload all command sequences, software updates, and emergency protocols for the full blackout period. (2) **Mars-side autonomy**: Surface assets switch to fully autonomous mode — rovers follow pre-planned survey routes, bases operate on cached instructions, all science data is stored locally in compressed format. (3) **Data triage**: Mars assets implement local prioritisation — only P0 events (safety alerts) trigger any attempt at transmission; everything else waits. (4) **Opportunistic relay**: If any LEO or Mars-orbital asset has a brief window to either L4 or L5, bundles are queued and transmitted during the first available micro-contact. (5) **Accept the gap**: I should be honest — with 0 contact windows, there is simply no communication possible. The system is designed to survive this: buffers are sized for 30+ days of storage, autonomous operations are pre-planned, and the conjunction period is predictable (it can be calculated years in advance from orbital mechanics). The DTN architecture is specifically designed for this — bundles wait, they don't expire if their lifetime is set appropriately (P2 bundles with 30-day lifetime survive a 14-day conjunction).

---

### C17. "What happens when all neighbors of a node have poor link quality — say, link quality below 0.3 for every possible next hop?"

The RL agent's action space includes STORE, which is the correct response when all forward paths are degraded. The agent has learned (through the reward function penalising drops) that storing is preferable to forwarding over a poor link. Specifically: (1) The agent selects STORE and waits for link conditions to improve. Orbital dynamics guarantee that link quality varies — a poor link at this orbital geometry will improve as the planets move. (2) The bundle's lifetime is checked — if remaining lifetime is sufficient to wait, the bundle is queued for the next contact window. (3) If the bundle's deadline is approaching and no link is improving, the agent may select SPLIT to fragment the bundle across multiple marginal paths, increasing the probability that at least some fragments arrive. (4) For P0/P1 bundles, the policy engine overrides the RL agent and forces transmission on the least-bad RF link (Ka-band or UHF), since some chance of delivery is better than guaranteed expiry. (5) If the node's buffer is filling up with stored bundles, Custody Refusal signals are sent upstream, redirecting traffic to alternate paths through other nodes. The key insight is that "poor link quality" is a temporary condition in a dynamic orbital environment — the RL agent learns the temporal pattern of link quality variation and optimises when to store vs. forward.

---

### C18. "How does the simulator validate that routing policy changes actually improve performance?"

AETHERIX uses an A/B testing framework within the simulation: (1) **Baseline run**: Execute the simulation with the current policy set over a full synodic period (780 days), recording delivery rate, average latency, hop count, drop rate, and energy consumption per priority level. (2) **Treatment run**: Execute the same simulation with the modified policy, using the same random seed and initial conditions for reproducibility. (3) **Comparison**: The difference in metrics between baseline and treatment is the policy's effect. Statistical significance is assessed by running each configuration 10+ times with different seeds and computing confidence intervals. (4) **Regression tests**: A set of scenario-specific tests verify that policy changes don't break edge cases: conjunction handling, buffer overflow, P0 pre-emption, energy budget enforcement. These tests have fixed expected outcomes (e.g., "P0 bundle must never be dropped"). (5) **CGR baseline**: Every RL/policy configuration is also compared against the CGR fallback. If the modified policy performs worse than CGR on any priority level, it is flagged for review. This methodology follows standard reinforcement learning evaluation practice — the simulator is the test environment, and reproducibility is ensured through fixed seeds and version-controlled configs.

---

### C19. "What is the QBER threshold and why specifically 11%?"

QBER (Quantum Bit Error Rate) is the fraction of errors in the sifted key after BB84 basis reconciliation. The 11% threshold comes from the Shor-Preskill security proof (2000), which showed that BB84 is secure against arbitrary quantum attacks (including collective and coherent attacks) when QBER < 11%. The proof works by showing that below this threshold, the amount of information an eavesdropper could have gained is bounded by `h(QBER)` (binary entropy), and privacy amplification can compress the key by this amount to produce a perfectly secret key. Above 11%, the binary entropy exceeds 0.5, and the secret key rate `1 − h(QBER)` becomes negative — meaning even after privacy amplification, no secret key can be extracted. The number 11% specifically comes from the intersection where `h(p) = 1` (approximately p = 0.1100). This is not an engineering parameter that can be relaxed — it is a fundamental information-theoretic bound. In practice, AETHERIX aborts QKD at QBER ≥ 10% (slightly below 11%) to provide a safety margin for finite-key effects and implementation imperfections that the asymptotic proof does not account for.

---

### C20. "How do you prevent cascading drops during congestion — where one node's drops cause downstream nodes to become congested too?"

Cascading congestion is a real risk in store-and-forward networks. AETHERIX prevents it through four mechanisms: (1) **Back-pressure via Custody Refusal**: When a node's buffer exceeds 90%, it sends Custody Refusal signals upstream. The upstream node stops forwarding to that destination and selects alternate paths. This is analogous to IEEE 802.3x pause frames but at the DTN layer. (2) **Adaptive rate limiting**: The policy engine tracks per-neighbor forwarding rates. If a neighbor's buffer is reported as >80% (via periodic status messages), the forwarding rate to that neighbor is throttled to 50% of the contact window capacity, preventing the neighbor from being overwhelmed. (3) **Priority isolation**: Each priority level has a guaranteed buffer share (5%/15%/40%/25%/15%). A P4 burst cannot consume P0 buffer space, so congestion in low-priority queues does not affect critical traffic. (4) **RL reward shaping**: The reward function penalises forwarding to a congested node more heavily than storing: the agent receives a negative reward if the next-hop's buffer occupancy (observed from recent status messages) exceeds 80%. This trains the agent to avoid creating congestion in the first place. The combination of protocol-level back-pressure, policy-level isolation, and RL-level avoidance provides three independent layers of protection against cascading failure.
